...
describe("useGenerateVideo", () => {
  ...
  it("generates a video and adopts the returned conversation", async () => {
    server.use(
      http.post("http://localhost/api/conversations/c1/videos", () =>
        HttpResponse.json(conversationResponse),
      ),
    );

    const { result } = renderHook(() => useGenerateVideo(gateway()), {
      wrapper: buildQueryWrapper(),
    });

    await result.current.mutateAsync({ conversationId: "c1", prompt: "um pôr do sol" });
